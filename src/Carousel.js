...
const Carousel = () => {
    ...
    return (
        <div className="relative w-full h-80 bg-cover bg-center" style={{
            ...
            <h1 className="absolute top-10 left-1/2 transform -translate-x-1/2 text-3xl text-gray-800 font-bold z-10">
                Experiências Compartilhadas
            </h1>
            <div className="absolute inset-x-0 top-20 flex justify-center">
                <div className="overflow-hidden rounded-lg w-3/4 md:w-1/2">
                    <div
                        className="flex transition-transform duration-300"
                        style={{ transform: `translateX(-${currentIndex * 100}%)` }}
                    >
                        {slides.map((slide, index) => (
                            <div key={index} className="flex-shrink-0 w-full h-48 flex flex-col items-center justify-center text-center p-6 bg-white shadow-md rounded-lg mx-2">
                                <h4 className="text-lg font-semibold text-gray-800">{slide.text}</h4>
                                <p className="mt-2 text-sm text-gray-600">{slide.name}</p>
                            </div>
                        ))}
                    </div>
                </div>
